...
@learn_with(TriangulatingLearner, bounds=((-1, 1), (-1, 1)))
...
@learn_with(TriangulatingLearner, bounds=((-1, 1), (-1, 1), (-1, 1)))
...
@pytest.mark.xfail
@run_with(Learner2D, TriangulatingLearner)
...
@run_with(xfail(Learner1D), Learner2D, TriangulatingLearner)
...
@run_with(Learner1D, xfail(Learner2D), TriangulatingLearner, AverageLearner)
...
@run_with(xfail(Learner1D), xfail(Learner2D), xfail(TriangulatingLearner), AverageLearner)
...
@run_with(Learner1D, Learner2D, TriangulatingLearner, AverageLearner)
...
# XXX: This *should* pass (https://gitlab.kwant-project.org/qt/adaptive/issues/84)
#      but we xfail it now, as Learner2D will be deprecated anyway
@run_with(Learner1D, xfail(Learner2D), xfail(TriangulatingLearner))
...
@pytest.mark.xfail
@run_with(Learner1D, Learner2D, TriangulatingLearner)
...
def test_faiure_case_TriangulatingLearner():
    log = [
        ('ask', 4),
    ...
    learner = TriangulatingLearner(lambda *x: x, bounds=[(-1, 1), (-1, 1), (-1, 1)])
